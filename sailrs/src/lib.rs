...
/// Loads Sail files from `sail.json`.
///
/// Parses supplied Sail files and returns the Sail AST
pub fn parse_sail_files<P: AsRef<Path>>(config_path: P) -> Result<sail_ast::Ast, Error> {
    ...
    RT.lock().execute(move |rt| {
        ...
        Ok(sail_ast::Ast::from_value(sail))
    })?
}
